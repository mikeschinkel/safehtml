...
// InnocuousId is an innocuous Id generated by IdentifierSanitized when passed an
// unsafe Id.
var innocuousId = `invalid:`

// IdentifierSanitized returns an Identifier which is either valid, or if invalid
// is replaced by an inoccuous ID in the form `invalid:<uuid>`.
func IdentifierSanitized(id string) Identifier {
	if !isSafeIdentifier(id) {
		return Identifier{innocuousId + uuid.New().String()}
	}
...
var safeIdentifierPattern = regexp.MustCompile(`^[\p{L}\p{N}_\-.:]+$`)

// isSafeIdentifier matches an identifier that only contains letters, numbers,
// and/or the common special characters.
// TODO: Verify that this regex pattern is secure
func isSafeIdentifier(id string) bool {
	return safeIdentifierPattern.MatchString(id)
}
